...
$('select.link-select').on('change', function() {
		var selectVal = $(this).val();
		if (selectVal != "") {
			var urlVlaues = selectVal.split("|");
			if (urlVlaues[1].length != 0 ){
				$(this.form).attr("action",urlVlaues[0]);
				$(this.form).attr("target",urlVlaues[1]);
				$(this.form).submit();
			}else{
				window.location.assign(urlVlaues[0]);
			}
		}
	});
    },
