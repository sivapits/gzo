...
class tx_feeditadvanced_tceforms extends t3lib_TCEforms_fe {
	/**
	 * Image path for Forms on page frontend editing mode
	 * @var	string
	 */
	var $imagePath = '';

	/**
	 * Path "back" to the main folder
	 * @var	string
	 */
	var $backPath = '';
	...
	function getDynTabMenu($menuItems,$identString,$toggle=0,$foldout=FALSE,$newRowCharLimit=50,$noWrap=1,$fullWidth=FALSE,$defaultTabIndex=1)	{
		...
		if (is_array($menuItems)) {
			...
			foreach($menuItems as $index => $def) {
				// Need to add one so checking for first index in JavaScript is different than if it is not set at all.
				$index+=1;
